...
const url = process.env.NODE_ENV === 'development' ? "http://localhost:8080/api" : "https://utbildning.miljonbemanning.se/api";
    ...
    async (error: AxiosError) => {
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
            ...
            try {
                ...
                const response = await axios.post<AuthResponse>(`${url}/auth/refresh`, {
                ...
                axiosInstance.defaults.headers['X-Refresh-Token'] = newRefreshToken;
